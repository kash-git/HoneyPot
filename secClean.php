        ...
        if(mysqli_num_rows($queryResSel)>0){
            while($row  = mysqli_fetch_assoc($queryResSel)){
                ...
                if($q >= $quantity){
                    $newQuant = $q - $quantity;
                    $totAmt = $quantity*699;
                    $book = "INSERT INTO orders VALUE('$quantity', '$totAmt','32 oz. Tile and Stone Cleaner','$name','$address','$pin','$mobile','$email')";
                    ...
                    echo "<p>Product : 32 oz. Tile and Stone Cleaner</p>";
                    echo "<p>Name : ".$name."</p>";
